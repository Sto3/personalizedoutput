/**
 * Restore Emma with ORIGINAL PERFECTED audio (santa_v4.mp3)
 * Only update: CTA order (urgency at top)
 * Keep: Original script, original 21.5s cut ending at "you did what was right"
 */
...
// ORIGINAL PERFECTED Emma config
const config = {
  name: 'emma',
  ...
  sparkleColor: [255, 200, 150],
  // ORIGINAL 21.5s cut - ends at "you did what was right"
  cutDuration: 21.5
};
...
async function main() {
  ...
  console.log('RESTORING EMMA - ORIGINAL PERFECTED VERSION');
  console.log('='.repeat(70));
  console.log('Using: santa_v4.mp3 (21.5s) - ends at "you did what was right"');
  console.log('Update: NEW CTA ORDER (urgency at top)');

  const framesDir = path.join(OUTPUT_DIR, 'frames-emma');
  // USE ORIGINAL PERFECTED AUDIO
  const santaAudioOriginal = path.join(AUDIO_DIR, 'santa_v4.mp3');
  const santaAudioProcessed = path.join(AUDIO_DIR, 'santa_emma_processed.mp3');

  if (!fs.existsSync(santaAudioOriginal)) {
    console.error('ERROR: Original perfected audio santa_v4.mp3 not found!');
    process.exit(1);
  }
  ...
  console.log('[1/4] Using ORIGINAL perfected Santa audio...');
  const rawDuration = parseFloat(execSync(`ffprobe -v quiet -show_entries format=duration -of csv=p=0 "${santaAudioOriginal}"`).toString().trim());
  console.log(`  ✓ Original audio: santa_v4.mp3 (${rawDuration.toFixed(1)}s)`);

  // Process with fade-in only (audio is already 21.5s, perfect cut)
  execSync(`ffmpeg -y -i "${santaAudioOriginal}" -af "afade=t=in:st=0:d=1" "${santaAudioProcessed}"`, { stdio: 'pipe' });
  console.log(`  ✓ Processed with fade-in`);
  ...
  const videoOnly = path.join(OUTPUT_DIR, 'emma_video.mp4');
  ...
  console.log('[4/4] Adding ORIGINAL Santa audio...');
  ...
  console.log('');
  console.log('='.repeat(70));
  console.log('✓ EMMA RESTORED - ORIGINAL PERFECTED VERSION');
  console.log('='.repeat(70));
  console.log(`File: ${config.filename}`);
  console.log(`Size: ${(stats.size / 1024 / 1024).toFixed(2)} MB`);
  console.log(`Duration: ${finalDuration.toFixed(1)}s`);
  console.log(`Santa audio: 21.5s (ends at "you did what was right")`);
  console.log('');
  console.log('CTA ORDER (NEW):');
