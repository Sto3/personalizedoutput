/**
 * Regenerate all videos (except Emma) with BRIGHT CTA screen
 * - Brighter background for CTA phase
 * - More compelling closing screen
 */
...
const VIDEOS = [
  {
    ...
    santaScript: `Ho ho ho, Elijah. This is Santa Claus. Your mom told me about the kind things you do, Elijah. How you hold the door for people. How you share your snacks. How you help classmates who are struggling. She told me all of it. And Elijah, here's what I want you to know. The most powerful kindness is the kind you do when you think no one is watching. Because that's when you know it comes from your heart. You asked your mom why you should be nice if nobody cares. But Elijah, people do care. Your mom sees it. And every person you've been kind to felt it. You made their day better. The world needs more people like you, Elijah. Don't ever stop being kind.`
  },
  {
    name: 'sofia',
...
function drawBackground(ctx, warmth, colors, isCTA = false) {
  ...
  if (isCTA) {
    // BRIGHT CTA background - festive red/burgundy gradient
    gradient.addColorStop(0, 'rgb(120, 30, 50)');  // Rich burgundy
    gradient.addColorStop(0.5, 'rgb(140, 40, 60)'); // Brighter red
    gradient.addColorStop(1, 'rgb(80, 20, 35)');   // Deep burgundy
  } else if (warmth > 0.5) {
...
async function createVideo(config) {
  ...
  console.log(`CREATING: ${config.childName} - BRIGHT CTA`);
  console.log('='.repeat(70));
  ...
  // Extra sparkles for CTA
  const ctaSparkles = Array(50).fill(null).map(() => new Sparkle([255, 215, 0])); // Gold sparkles
  ...
  for (let frame = 0; frame < totalFrames; frame++) {
    ...
    if (warmth > 0.5) {
      for (const sparkle of sparkles) { sparkle.update(); sparkle.draw(ctx); }
    }

    // Extra gold sparkles during CTA
    if (isCTA) {
      for (const sparkle of ctaSparkles) { sparkle.update(); sparkle.draw(ctx); }
    }
    ...
    switch (phase) {
      ...
      case 'cta':
        // BRIGHT CTA with more vibrant colors
        drawText(ctx, "We'll create one for your child today", 72, HEIGHT * 0.18);
        drawText(ctx, "Ready before Christmas \u{1F384}", 68, HEIGHT * 0.30, { color: '#FFD700' }); // Gold
  ...
  console.log(`✓ COMPLETE: ${config.filename} (${finalDuration.toFixed(1)}s)`);

  return { name: config.name, filename: config.filename, duration: finalDuration };
}

async function main() {
  console.log('='.repeat(70));
  console.log('REGENERATING ALL WITH BRIGHT CTA SCREEN');
  console.log('='.repeat(70));
  console.log('- Rich burgundy background for CTA');
  console.log('- Extra gold sparkles');
  console.log('- Gold "Ready before Christmas" text');
  ...
  console.log('');
  console.log('='.repeat(70));
  console.log('ALL COMPLETE - BRIGHT CTA');
  console.log('='.repeat(70));
  for (const r of results) {
    console.log(`✓ ${r.filename}: ${r.duration.toFixed(1)}s`);
  }

  // Open all for review
  for (const r of results) {
    execSync(`open "${path.join(FINAL_VIDEOS_DIR, r.filename)}"`);
  }
}
