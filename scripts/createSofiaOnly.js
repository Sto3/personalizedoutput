/**
 * Create Sofia video only (extended cut point)
 */
...
async function main() {
  ...
  console.log('CREATING SOFIA VIDEO (EXTENDED CUT)');
  console.log('='.repeat(70));

  const framesDir = path.join(OUTPUT_DIR, 'frames-sofia');
  const santaAudioRaw = path.join(AUDIO_DIR, 'santa_sofia.mp3');
  const santaAudioProcessed = path.join(AUDIO_DIR, 'santa_sofia_processed.mp3');
  ...
  console.log(`  ✓ Generated: santa_sofia.mp3 (${rawDuration.toFixed(1)}s)`);
  ...
  const videoOnly = path.join(OUTPUT_DIR, 'sofia_video.mp4');
  ...
  console.log('');
  console.log('='.repeat(70));
  console.log('✓ SOFIA VIDEO COMPLETE');
  console.log('='.repeat(70));
  console.log(`File: ${config.filename}`);
  console.log(`Size: ${(stats.size / 1024 / 1024).toFixed(2)} MB`);
  console.log(`Duration: ${finalDuration.toFixed(1)}s`);
  console.log(`Cut point: ${config.cutDuration}s (should complete "And Sofia, that is exactly what you do")`);
