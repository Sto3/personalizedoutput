/**
 * Create James video - FIXED
 * - "fortunate" instead of "lucky"
 * - Regenerate audio with new script
 * - Proper cut point (not cutting off final "James")
 */
...
const config = {
  ...
  // FIXED: "fortunate" instead of "lucky"
  ...
  // Will determine after audio generation
  cutDuration: null
};
...
async function main() {
  ...
  console.log('CREATING JAMES VIDEO - FIXED');
  console.log('='.repeat(70));
  console.log('FIX: "fortunate" instead of "lucky"');
  console.log('FIX: Using FULL audio (no cutoff)');
  ...
  console.log('[1/4] Generating NEW Santa audio (with "fortunate")...');
  ...
  // Use FULL audio - no cutting!
  config.cutDuration = rawDuration;
  execSync(`ffmpeg -y -i "${santaAudioRaw}" -af "afade=t=in:st=0:d=1" "${santaAudioProcessed}"`, { stdio: 'pipe' });
  console.log(`  ✓ Using FULL audio: ${rawDuration.toFixed(1)}s (no cutoff)`);
  ...
  console.log('');
  console.log('='.repeat(70));
  console.log('✓ JAMES VIDEO COMPLETE - FIXED');
  console.log('='.repeat(70));
  console.log(`File: ${config.filename}`);
  console.log(`Size: ${(stats.size / 1024 / 1024).toFixed(2)} MB`);
  console.log(`Duration: ${finalDuration.toFixed(1)}s`);
  console.log('FIXES:');
  console.log('  ✓ "fortunate" instead of "lucky"');
  console.log(`  ✓ FULL audio used: ${rawDuration.toFixed(1)}s (no cutoff)`);
