/**
 * Holiday Reset Form Schema
 *
 * JSON-serializable form schema for Holiday Relationship Reset planner.
...
export function getHolidayResetFormSchema(): FormSchema {
  return {
    productId: 'holiday_relationship_reset',
    productName: 'Holiday Relationship Reset Planner',
    version: '1.0.0',
        ...
        questions: [
          {
            ...
            options: [
              { value: 'gentle', label: 'Gentle', description: 'Compassionate, soft, nurturing' },
              { value: 'balanced', label: 'Balanced', description: 'Warm but straightforward' },
              { value: 'direct', label: 'Direct', description: 'Tell it like it is, no sugarcoating' }
            ],
            required: true
          },
          {
...
export function normalizeHolidayResetAnswers(rawAnswers: Record<string, unknown>): Record<string, unknown> {
